...
export default function AlphaZero() {
  ...
  // Track connStatus via ref so getCandles/runSelect don't recreate on every WS state flip
  ...
  // Fetch candles — stable callback, reads connStatus from ref to avoid re-render cascade
  ...
  const openModal=async(stock)=>{
    ...
    const dataSource=connRef.current==="live"?"Real NSE data via yfinance":"Browser simulation (start backend for real data)";
    const pats=detectCandlePatterns(cdl);
